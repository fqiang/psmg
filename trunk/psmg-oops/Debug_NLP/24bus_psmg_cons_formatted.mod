...
#also, constraints like this should be moved to each sub-block, ie. the Contingency[c] block for optimal result, because it is not linking between each Contingency cases, by modeling this way, PSMG will spend time to compute these constraints, event thought most of block intersection for Jacobian and Hessian are zeros matrix.
#Therefore, the rule of thumn is that, *only constraints linking sub-blocks below should be put in the ra upper level
