...
SyntaxNode::SyntaxNode(int code, SyntaxNode *val1, SyntaxNode *val2,
		SyntaxNode* val3) :
		opCode(code) {
	if (val1 != NULL) {
	...
	LOG(
			"SyntaxNode constructor  --- "<<values.size()<<"<- opCode["<<opCode<<"] val1:["<<val1<<"] val2:["<<val2<<"] val3:["<<val3<<"]");
}

SyntaxNode::~SyntaxNode() {
	for (vector<SyntaxNode*>::iterator it = values.begin(); it != values.end();
			it++) {
		delete *it;
	}
	values.clear();
...
ModelComp *SyntaxNode::findModelComp() const {
	...
	if (opCode == IDREF) {
		const SyntaxNodeIDREF *onref =
				dynamic_cast<const SyntaxNodeIDREF*>(this);
		return onref->ref;
	}
	return NULL;
}
...
string SyntaxNode::getArgumentList() const {
	...
	if (on == NULL) {
		cout
				<< "WARNING: This is an IDREF SyntaxNode not of type SyntaxNodeIDREF\n";
		if (nchild() > 0) {
...
int SyntaxNode::calculateVarDim() {
	...
	if (this->opCode == LBRACE) {
		...
		LOG(
				"opCode["<<this->opCode<<"] was not yet implemented --- calculateVarDim");
		assert(false);
	}
	return dim;
}
...
int SyntaxNode::calculateSetDim() {
	...
	if (this->opCode == DEFINED) { //OpNode
		...
		LOG(
				"opCode["<<this->opCode<<"] was not yet implemented --- calculateSetDim");
		assert(false);
	}
...
void SyntaxNode::calculateParamIndicies(vector<string>& paramIndiciesDummy,
	...
	if (this->opCode == IDREF) {
		...
		assert(this->values.size()==1);
		this->values[0]->calculateParamIndicies(paramIndiciesDummy,
				paramIndiciesComp, paramIndiciesMap);
	} else if (this->opCode == COMMA) {
		LOG("this ListNode size["<<this->values.size()<<"]");
		vector<SyntaxNode*>::iterator i;
		for (i = this->values.begin(); i != this->values.end(); i++) {
			(*i)->calculateParamIndicies(paramIndiciesDummy, paramIndiciesComp,
					paramIndiciesMap);
		}
		...
		LOG(
				"opCode["<<this->opCode<<"] was not yet implemented -- calculateNumParamIndicies");
		assert(false);
	}
}
...
Set* SyntaxNode::calculateSetValue(ModelContext* context) {
	...
	if (this->opCode == DEFINED) {
		...
		if (nodeIdRef->ref->type == TSET) {
			...
			Param* aParam = static_cast<Param*>(context->getCompValue(
					nodeIdRef->ref));
		...
		if (this->values[1]->opCode == -99) {
			...
			SyntaxNodeIDREF* refn =
					static_cast<SyntaxNodeIDREF*>(this->values[1]->values[0]);
			assert(refn->ref->type ==TPARAM);
			Param* aParam =
					static_cast<Param*>(context->getCompValue(refn->ref));
			assert(aParam->card==1);
		...
		LOG(
				"calculateSetValue -- op not yet implmented -- opCode["<<this->opCode<<"]");
		assert(false);
	}
	return value;
}
...
Set* SyntaxNode::evalSet(ModelContext* context) {
	...
	SyntaxNodeIDREF* refn =
			static_cast<SyntaxNodeIDREF*>(this->values[0]->values[1]);
...
double SyntaxNode::calculateParamValue(hash_map<string, string>& dummyValueMap,
		hash_map<string, ModelComp*>& paramIndicies, ModelContext* context) {
	LOG(
			"calculateParamValue  ---  opCode["<<this->opCode<<"]  "<<this->print());
	double rval = 0;
	if (this->opCode == DEFINED) {
		rval = this->values[0]->calculateParamValue(dummyValueMap,
				paramIndicies, context);
	} else if (this->opCode == COMMA) {
		rval = this->values[0]->calculateParamValue(dummyValueMap,
				paramIndicies, context);
	} else if (this->opCode == -99) {
		...
		bool cond = this->values[0]->evalBool(dummyValueMap, paramIndicies,
				context);
		rval = cond == true ?
				this->values[1]->calculateParamValue(dummyValueMap,
						paramIndicies, context) :
				this->values[2]->calculateParamValue(dummyValueMap,
						paramIndicies, context);
		...
		if (this->values.size() == 1) {
			...
			double left = this->values[0]->calculateParamValue(dummyValueMap,
					paramIndicies, context);
			double right = this->values[1]->calculateParamValue(dummyValueMap,
					paramIndicies, context);
			rval = left - right;
		} else {
		...
		IDNode* idNode =
				static_cast<IDNode*>(this->values[0]->values[0]->values[0]);
		SyntaxNodeIDREF* refNode =
				static_cast<SyntaxNodeIDREF*>(this->values[0]->values[0]->values[1]);
		...
		for (; it != aSet->setValues_data_order.end(); it++) {
			...
			sumVal += this->values[1]->evalTerm(dummyValueMap, paramIndicies,
					context);
			context->removeDummySetValueMapTemp(dummy);
		}
		rval = sumVal;
		...
		assert(
				this->values[0]->values[0]->values[0]->values[0]->opCode == IDREF);
		SyntaxNodeIDREF* refNode =
				static_cast<SyntaxNodeIDREF*>(this->values[0]->values[0]->values[0]->values[0]);
		Set* aSet = static_cast<Set*>(context->getCompValue(refNode->ref));
		rval = (double) aSet->getCard();
	} else if (this->opCode == 47) //'/' operator
			{
		double numerator = this->values[0]->calculateParamValue(dummyValueMap,
				paramIndicies, context);
		double denominator = this->values[1]->calculateParamValue(dummyValueMap,
				paramIndicies, context);
		rval = numerator / denominator;
	} else {
		LOG(
				"calculateParamValue ---  opCode["<<this->opCode<<"] not yet implemented");
		assert(false);
	}
	return rval;
}

bool SyntaxNode::evalBool(hash_map<string, string>& dummyValueMap,
		hash_map<string, ModelComp*>& paramIndicies, ModelContext* context) {
	LOG("evalBool  ---  opCode["<<this->opCode<<"]  "<<this->print());
	bool rval = false;
	if (this->opCode == EQ) {
		double r0 = this->values[0]->evalTerm(dummyValueMap, paramIndicies,
				context);
		LOG("--- value for r0["<<r0<<"]");
		double r1 = this->values[1]->evalTerm(dummyValueMap, paramIndicies,
				context);
		LOG("--- value for r1["<<r1<<"]");
		if (r0 == r1) {
			rval = true;
...
double SyntaxNode::evalTerm(hash_map<string, string>& dummyValueMap,
		hash_map<string, ModelComp*>& paramIndicies, ModelContext* context) {
	...
	if (this->opCode == IDREF) {
		...
		IDNode* in =
				static_cast<IDNode*>(this->values[0]->values[0]->values[0]->values[0]);
		string dummy = in->id();
		ModelComp* comp = paramIndicies.find(dummy)->second;
		Set* theSet = static_cast<Set*>(context->getCompValue(comp));
		rval =
				(double) (theSet->getSetOrder(dummyValueMap.find(dummy)->second));
...
void SyntaxNode::foreachSetValue(vector<ModelComp*> comps,
		vector<string>& dummyVars, Set* aSet, ModelContext* rowContext,
		vector<double>& jcobs, ModelContext* colContext) {
	...
	for (; it_setval != aSet->setValues_data_order.end(); it_setval++) {
		...
		assert(
				sval->valueList.size()==comps.size()&&comps.size()==dummyVars.size());
		...
		for (; it_comp != comps.end(); it_comp++, it_dummyVar++, it_sval++) {
			rowContext->addDummySetValueMapTemp((*it_dummyVar), *it_comp,
					*it_sval);
		}
...
bool SyntaxNode::isDepend(vector<ModelComp*> varComps) {
	...
	if (this->opCode == 0) {
		...
		for (vector<ModelComp*>::iterator it = varComps.begin();
				it != varComps.end(); it++) {
			if (*it == comp) {
				return true;
			}
...
void SyntaxNode::handleSum(ModelContext* rowContext, vector<double>& jcobs,
		ModelContext* colContext) {
...
//	foreachSetValue(comps, dummyVars, aSet, rowContext, jcobs, colContext);
	...
	SyntaxNodeIDREF* consSumRef =
			static_cast<SyntaxNodeIDREF*>(cons_sum_node->values[1]);
	if (col_index_node != NULL
			&& consSumRef->ref
					== static_cast<SyntaxNodeIDREF*>(col_index_node->values[1])->ref) {
		LOG(
				"EQUAL - CONS SUM Indexing["<<cons_sum_node->print()<<"] Col Model Indexing["<<col_index_node->print()<<"]");
		string dummyVar = (colContext->dummySetMap.begin())->first;
		rowContext->addDummySetValueMapTemp(dummyVar,
				(colContext->dummySetMap.begin())->second,
				(colContext->dummyValueMap.begin())->second);
		this->values[1]->evalDiff(rowContext, colContext, jcobs);
		rowContext->removeDummySetValueMapTemp(dummyVar);
	} else {
		LOG(
				"NOT - CONS SUM Indexing["<<cons_sum_node->print()<<"] Col Model Indexing["<<col_index_node->print()<<"]");
		if (this->values[1]->isDepend(colEm2->localVarComps)) {
			...
			ostringstream oss;
			oss //<< rowContext->getModelDummyValAsKey() //don't need this one since context is hirechical structured! -Feng
			<< rowContext->getConsDummyValAsKey() << (void*) ((this->values[0]))
					<< colContext;
			string hashKey = oss.str();
			if (!rowContext->getCalcTempSet(hashKey, comps, &aSet, dummyVars)) {
				this->values[0]->calcTempSetComp(rowContext, comps, &aSet,
						dummyVars);
				LOG("sum { dummyVar } over "<<aSet->toString());
				LOG(
						"-- Add Temp Set - hashKey["<<hashKey<<"] overSet "<<aSet->toString());
			...
			vector<SetValue*>::iterator it_setval =
			...
			for (; it_setval != aSet->setValues_data_order.end(); it_setval++) {
				...
				vector<ModelComp*>::iterator it_comp = comps.begin();
				vector<string>::iterator it_dummyVar = dummyVars.begin();
				for (; it_comp != comps.end();
						it_comp++, it_dummyVar++, it_sval++) {
					rowContext->addDummySetValueMapTemp((*it_dummyVar),
							*it_comp, *it_sval);
				}
			...
			LOG(
					"["<<this->values[1]->print()<<"] not depend on ["<<colEm2->model->name<<"]'s localvar");
		}
	}
}

void SyntaxNode::getVarKey(string& varKey, SyntaxNodeIDREF* refn,
		ModelContext* rowContext) {
	ModelComp* comp = refn->ref;
...
void SyntaxNode::evalDiff(ModelContext* rowContext, ModelContext* colContext,
		vector<double>& jcobs) {
	LOG(
			"evalDiff --- opCode["<<this->opCode<<"] -- jcobs.size["<<jcobs.size()<<"] -- "<<this->print());
	if (this->opCode == ASSIGN) {
		...
		if (values.size() == 1) {
			...
			assert(
					jcobs.size() == leftJcobs.size() && leftJcobs.size() == rightJcobs.size());
		...
		if (comp->type == TPARAM) {
			...
			while (!found && it_jcobs != jcobs.end()) {
				...
				if (varComp == comp) {
					if (!currVarKeyReady) {
						LOG(
								"["<<varComp->id<<"] matches - now calculate varKey");
					...
					vector<VarValue*>::iterator it2 =
							(*it_var)->varList.begin();
					...
					while (!found && it2 != (*it_var)->varList.end()) {
						...
						if (varKey.compare(currVarKey) == 0) {
							*it_jcobs = 1;
							found = true;
							LOG(
									"["<<varComp->id<<"] matches ["<<varKey<<"] matches ");
						} else {
							LOG(
									"["<<varComp->id<<"] matches ["<<varKey<<"] not match ");
							*it_jcobs = 0;
						}
						it_jcobs++;
						it2++;
					...
					LOG(
							"["<<varComp->id<<"] not match - go to next varComp -- card["<<(*it_var)->card<<"]");
		...
		} else if (!this->isZeroVector(leftJcobs)
				&& this->isZeroVector(rightJcobs)) {
		...
		LOG(
				" -- opCode["<<opCode<<"] is not implmeneted yet for automatic differentiation!");
		assert(false);
	}
}
...
void SyntaxNode::multVectorScalar(vector<double>& v, double scalar,
...
void SyntaxNode::plusVector(vector<double>& left, vector<double>& right,
		vector<double>& result) {
...
void SyntaxNode::minusVector(vector<double>& left, vector<double>& right,
		vector<double>& result) {
...
void SyntaxNode::calcTempSetComp(ModelContext* context,
		vector<ModelComp*>& comps, Set** theSet, vector<string>& dummyNames) {
...
//		*theSet = setCopy;
		...
		*theSet = newSet;
		...
		for (vector<SyntaxNode*>::iterator it = this->values.begin();
				it != this->values.end(); it++) {
			LOG("COMMA List -- handling ["<<(*it)->print()<<"]");
			(*it)->calculateCompsDummyNames(context, comps, dummyNames);
		}
		assert(
				comps.size()==this->values.size()&&comps.size()==dummyNames.size());

		*theSet = new Set(this->values.size(), "TMP_UP" + context->moveUpLevel);
		for (vector<ExpandedModel2*>::iterator em2LevelIt =
				em2LevelList.begin(); em2LevelIt != em2LevelList.end();
				em2LevelIt++) {
			this->calculateSet((*em2LevelIt)->context, comps, dummyNames,
					*theSet);
		}
	} else {
...
void SyntaxNode::calculateCompsDummyNames(ModelContext* context,
		vector<ModelComp*>&comps, vector<string>& dummyNames) {
	string dummyName = static_cast<IDNode*>(this->values[0])->id();
	ModelComp* aComp = static_cast<SyntaxNodeIDREF*>(this->values[1])->ref;
	comps.push_back(aComp);
	dummyNames.push_back(dummyName);
	LOG(
			"calculateCompsDummyNames -- adding ModelComp["<<aComp->id<<"] dummyName["<<dummyName<<"]");
}

void SyntaxNode::calculateSet(ModelContext* context, vector<ModelComp*>& comps,
		vector<string>& dummyNames, Set* theSet) {
	LOG("calculateSet --- opCode["<<this->opCode<<"]  "<<this->print());
	assert(this->opCode == COMMA);
	vector<string> valueList;
	for (vector<string>::iterator it = dummyNames.begin();
			it != dummyNames.end(); it++) {
		string dummyName = *it;
		string dummyVal = context->getDummyValue(dummyName);
		LOG(
				"calculateSet -- dummyName["<<dummyName<<"]  --->  dummyVal["<<dummyVal<<"]");
...
//		}
		...
		string key = "";
		for (vector<SyntaxNode*>::iterator it = refn->values.begin();
				it != refn->values.end(); it++) {
		...
		LOG(
				"aParam["<<aParam->getName()<<"] dummy["<<idn->id()<<"] dummyVal["<<dummyVal<<"] --> rval["<<rval<<"]");
	} else if (this->opCode == ID) {
		IDNode* idn = static_cast<IDNode*>(this);
		...
		LOG(
				"evalTerm -- single -- opCode["<<this->opCode<<"] not yet implmemented -- value_size["<<this->values.size()<<"]");
		assert(false);
	}
...
string SyntaxNode::setOrgSetComp(ModelComp** orgSetComp) {
	...
	*orgSetComp = static_cast<SyntaxNodeIDREF*>(this->values[1])->ref;
	...
	LOG(
			"end setOrgSetComp - dummy["<<dummy<<"] ref["<<(*orgSetComp)->id<<"|"<<*orgSetComp<<"]");
	return dummy;
}
...
SyntaxNode* SyntaxNode::find_var_ref_in_context(AmplModel *context, SyntaxNode *ref) {
	...
	if (!ret) {
		LOG(
				"--> not found in either current index or current context's modelComp");
		if (argNode) {
