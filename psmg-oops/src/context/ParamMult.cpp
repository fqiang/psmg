...
{
	TRACE("Create ParamMult - name["<<name<<"] numIndicies["<<numIndicies<<"]");
}
...
{
	TRACE("ParamMult - Add ParamValue name["<<name<<"] card["<<card<<"] ["<<indiciesKey<<" <= "<<value->toString()<<"]");
...
{
	TRACE("-- ParamMult::findParamValue - key["<<indiciesKey<<"]" ); //in"<<endl<<this->toString());
	PValue* rval = (this->paramValues.find(indiciesKey)->second);
	TRACE("-- ParamMult::findParamValue -- found  - ["<<rval<<"]");
	return rval;
}
