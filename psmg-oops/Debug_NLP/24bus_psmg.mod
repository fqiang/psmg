...
subject to KCLP{b in Bus}:
	sum{g in Generator: Location[g] == b} (PpowerGen[g]) - demScaleDemand*DemandP[b] 
	=
...
subject to KCLQ{b in Bus}:
	sum{g in Generator: Location[g] == b} (QpowerGen[g]) - demScaleDemand*DemandQ[b]
	=
...
subject to FlowLimitStart{l in LUT}:
	FlowStartP[l]^2 + FlowStartQ[l]^2 = slackStart[l];
subject to FlowLimitEnd{l in LUT}:
	FlowEndP[l]^2 + FlowEndQ[l]^2 = slackEnd[l];
...
block Contingency{c in Contingencies}: {
	...
	#KCL + P power
	subject to KCLP{b in Bus}:
		sum{g in Generator: Location[g] == b} (cPpowerGen[g]) - demScaleDemand*DemandP[b] 
		=
	...
	#KCL + Q power
	subject to KCLQ{b in Bus}:
		sum{g in Generator: Location[g] == b} (cQpowerGen[g]) - demScaleDemand*DemandQ[b]
		=
	...
	subject to FlowLimitStart{l in LUTDIFF}:
		cFlowStartP[l]^2 + cFlowStartQ[l]^2 = cslackStart[l];
	subject to FlowLimitEnd{l in LUTDIFF}:
		cFlowEndP[l]^2 + cFlowEndQ[l]^2 = cslackEnd[l];

	##########################################
	#Linking constraints to first stage
	##########################################
	subject to BusVolCons{b in PVBUS}: cVoltages[b] = Voltages[b];
	#noteA: duplicate temporary set created for {g in Generator: Location[g]!=RefBus and (Location[g] in PVBUS)}	
	subject to BusGenCons{g in Generator: Location[g]!=RefBus and (Location[g] in PVBUS) }: cPpowerGen[g] = PpowerGen[g];
}
